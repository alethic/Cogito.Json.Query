...
namespace Cogito.Json.Query.Internal
{
        ...
        where TValue : class
    {
        ...
        T Dereference<T>(WeakReference<T> reference)
            where T : class
        {
            return reference != null ? reference.TryGetTarget(out var o) ? o : null : null;
        }
            ...
            using (sync.BeginUpgradableReadLock())
            {
                ...
                if (l > MIN_CLEAN_INTERVAL + storage.Count)
                {
                    using (sync.BeginWriteLock())
                    {
                        ...
                        var curGeneration = GC.CollectionCount(0);
        ...
        {
            return TryRemove(item.Key, out var o);
        }
        ...
        {
            return TryRemove(key, out var o);
        }
